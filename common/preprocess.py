...
if __name__ == "__main__":
    ...
    need_to_annotate = main(args)

    while need_to_annotate:
        ...
        annotation_samples = np.arange(
            0,
            int(len(img_names)),
            int(int(len(img_names)) / ((int(len(img_names)) * int(args.percentage_if_gt)) / 100)),
        )

        total_annotations_required = len(annotation_samples)

        print(
            "----------------------------------------------------------------------------------------"
        )
        # TODO: Temporarily keeping this bypass check. Would implement something more dynamic later.
        # pdb.set_trace()
        if os.path.isdir("data/" + str(args.dataset) + "/annot/annot"):
            # need_to_annotate = False
            break

        print(
            "Assumimg {}% to be annotated, we would need to annotate {} frames per view".format(
                args.percentage_if_gt, total_annotations_required
            )
        )
        ...
        if not exists("configs/joints/" + str(args.dataset) + ".json"):
            print(
                "Uh oh! We did not find any keypoint information. Please tell us something more about which points you wish to track by left-clicking the points on this image"
            )
            # reading the image
            ...
            # print(
            #     "In total, ~{0:.2f} minutes are required to finish the initial labeling task.".format(
            #         (counter) * total_annotations_required / 33
            #     )
            # )
            ...
            print(
                "Decide the landmark connections and store them in the config. Don't worry, the landmark connections are just for visualization."
            )

            # cv2.imshow('image', img)
            font = cv2.FONT_HERSHEY_SIMPLEX
            for idx in range(len(landmark_coordinates_global)):
                str_to_print = str(idx) + " | " + landmark_str_global[idx]
                cv2.putText(
                    img, str_to_print, landmark_coordinates_global[idx], font, 1, (0, 0, 255), 2
                )
            ...
            tpl = tuple(lst)
            tpl_list = []
            for idx in range(int(len(tpl) / 2)):
                tpl_list.append(tpl[2 * (idx) : 2 * (idx + 1)])

            final_joint_connections = tuple(tpl_list)
            ...
            joints_data = {
                "num_joints": counter,
                "joint_connections": final_joint_connections,
                "joints_name": landmark_str_global,
                "range_scale": int(1),
                "R": np.eye(3).tolist(),
            }

            with open("configs/joints/" + str(args.dataset) + ".json", "w") as fid:
                fid.write(json.dumps(joints_data))
